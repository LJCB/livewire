<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 bg-white shadow-md dark:bg-gray-800 sm:rounded-lg p-4 mb-4">
            <div>
                <h5 class="mr-3 font-semibold dark:text-white">Oferta educativa</h5>
                <p class="text-gray-500 dark:text-gray-400">Consulta todos los cursos que tenemos disponibles para ti</p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between pb-4">
            ...
            <ul role="list" class="grid grid-cols-4 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3">
                @foreach ($courses as $course)
                <li class="col-span-1 flex flex-col divide-y divide-gray-200 rounded-lg bg-white text-center shadow">
                    <div class="flex flex-1 flex-col p-8">
                        <img class="mx-auto h-32 w-32 flex-shrink-0 rounded-full"
                            src="{{ asset('storage/course/cover/' . $course->id . '.png') }}"
                            alt="">
                        <h3 class="mt-6 text-md font-medium text-gray-900">{{$course->title}}</h3>
