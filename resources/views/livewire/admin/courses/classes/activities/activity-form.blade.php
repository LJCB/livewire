<div>
    ...
    <div class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
        ...
        <div class=" text-black  dark:text-white underline pt-2" >
            <a href="{{ route('admin.course.classes.activities', $activity->course_class_id) }}">Regresar al listado</a>
        </div>
    </div>
    @endif
   

    <form wire:submit.prevent="{{ $activity ? 'update' : 'store' }}">
        <input type="hidden" wire:model="activityId" value="{{ isset($activity) ?  $activity->id : 0}}">
        <input type="hidden" wire:model="classId" value="{{ isset($class) ?  $class->id : 0}}">
        <div class="mb-6">
            ...
            <textarea id="message" rows="4" wire:model='description'
                ...
                placeholder="Descripción del curso" required> {{ $activity ? $activity->description: '' }} </textarea>
            @error('description') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>

        <div class="mb-6">
            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="user_avatar">Archivo de la
                actividad</label>
            {{-- @if($activity)
            <div class=" flex items-center justify-center">
                <img src="{{ asset('storage/course/classes/'. .'cover/' . $course->id . '.png') }}" class="w-full h-auto max-w-xl rounded-lg" alt="image description">    
            </div>
            ...
            <input
                ...
                type="file" wire:model='activityFile'>
            @error('activityFile') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
