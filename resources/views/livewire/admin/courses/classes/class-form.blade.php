...
<div>
    ...
    <form wire:submit.prevent="{{ $class ? 'update' : 'store' }}">
        <div class="mb-6">
            <input type="hidden" wire:model="classId" value="{{ isset($class) ?  $class->id : 0}}">
            <input type="hidden" wire:model="classId" name="classId" value="{{ isset($class) ?  $class->id : 0}}">
            ...
            <input type="text" wire:model='title'
                ...
                placeholder="Titulo de curso" value="{{ $class ? $class->title : '' }}" required>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea id="message" rows="4" wire:model='description'
                ...
                placeholder="Descripción del curso" required> {{ $class ? $class->image: '' }} </textarea>
            @error('description') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>

        <div class="mb-6">
            <label for="videoUrl" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">URL de video</label>
            <input type="text" wire:model='videoUrl'
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Titulo de curso" value="{{ $class ? $class->videoUrl : '' }}" required>
        </div>
