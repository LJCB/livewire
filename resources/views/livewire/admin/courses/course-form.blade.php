<div>
    ...
    <form wire:submit.prevent="{{ $course ? 'update' : 'store' }}">
        <div class="mb-6">
            <input type="hidden" wire:model="courseId" name="courseId" value="{{ isset($course) ?  $course->id : 0}}">
            <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre del
                curso</label>
            <input type="title" id="title" name="title" wire:model='title'
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Titulo de curso" value="{{ $course ? $course->title : '' }}" required>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea id="message" rows="4" wire:model='description'
                ...
                placeholder="Descripción del curso" required> {{ $course ? $course->image: '' }} </textarea>
            @error('description') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>

        <div class="mb-6">
            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="user_avatar">Imagen del
                curso</label>
            @if($course)
            <div class=" flex items-center justify-center">
                <img src="{{ asset('storage/course/cover/' . $course->id . '.png') }}" class="w-full h-auto max-w-xl rounded-lg" alt="image description">    
            </div>
            ...
            <input
                ...
                aria-describedby="course_image" id="course_image" type="file" wire:model='courseImage'>
        </div>
